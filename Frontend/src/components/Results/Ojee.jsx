...
function Ojee() {
  ...
  <div className="flex flex-nowrap overflow-x-auto gap-1 sm:gap-3 px-0 sm:px-2 sm:mx-15">
    {provide.map((item) => (
      <div
        key={item.id}
        className="bg-amber-50 text-black w-45 sm:w-68 flex-shrink-0 rounded-lg shadow-md hover:shadow-lg transition duration-300 flex flex-col"
      >
        <figure className="px-0 pt-0 sm:px-2 sm:pt-2">
          <img
            src={item.image}
            alt={item.title}
            className="rounded-md object-cover object-top mt-2 mb-4 w-40 h-40 ml-2  h-14 sm:h-65 sm:w-60 "
          />
        </figure>
